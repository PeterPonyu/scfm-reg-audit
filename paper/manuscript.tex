...
\begin{figure}[!tbp]\centering
\fitfig{\input{figs/fig2_decisive_result.tex}}
...
\begin{figure}[!tbp]\centering
\fitfig{\input{figs/fig3_spec_sensitivity.tex}}
\caption{Confound-specification sensitivity. Full and non-degree estimates are joined within each
model/readout and tissue. The non-degree specification is reported co-primary and shows
null-dependent support.}
\label{fig:spec}
\end{figure}
...
\begin{figure}[!tbp]\centering
\fitfig{\input{figs/fig5_pertype_descriptive.tex}}
...
\begin{figure}[!tbp]\centering
\fitfig{\input{figs/fig4_pipeline_sensitivity.tex}}
...
\begin{figure}[!tbp]\centering
\fitfig{\input{figs/fig6_tf_probe.tex}}
\caption{TF-disjoint supervised probe. (A) Confound-adjusted test correlation per family on 135
held-out TFs; labels give the gene-label permutation $q_M$ and, for each FM family, the paired
sign-flip $q_{\mathrm{flip}}$ against the co-expression baseline. Green rows are supported against
the baseline at $q_{\mathrm{flip}}<0.05$ (UCE $0.0325$; random-init floor $0.0300$), and the
supported floor contrast prevents a model-specific reading of UCE. (B) The same readout across
confound subsets; recovery
