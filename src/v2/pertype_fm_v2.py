#!/usr/bin/env python
"""
scfm-reg-audit v2 — PER-CELL-TYPE FM readout (GPU) vs G_ATAC v2, gated annotation.
Completes the CPU per-type coexp (pertype_coexp_v2) with the FM half: for each confidently-typed
brain population, Geneformer EMBEDDING + ATTENTION graphs vs per-type regulatory truth, marginal
+ partial | per-type co-expression. Markers excluded from the pair set. Reports per-type + mean.
"""
...
rd = fr.FMReadout(genes, batch=8); rows = []
for t in atac_types:
    ...
    Gemb = fr.FMReadout.cos_graph(rd.geneformer(Xc_g, cids)); emb = Gemb[ii, jj]
    Gatt = rd.geneformer_attention(Xc_g, cids); at = Gatt[ii, jj]
    np.savez(f"{OUT}/brain_fmgraphs_{t}.npz", co=Gco, gf=Gemb, at=Gatt)  # cache for downstream confound+bootstrap analysis
    rows.append(dict(cell_type=t, n=int(len(cids)),
                     coexp_vs_atac=round(float(spearmanr(co, a).statistic), 4),
                     emb_vs_atac=round(float(spearmanr(emb, a).statistic), 4),
                     emb_partial=round(partial(emb, a, co), 4),
                     attn_vs_atac=round(float(spearmanr(at, a).statistic), 4),
                     attn_partial=round(partial(at, a, co), 4),
                     attn_vs_coexp=round(float(spearmanr(at, co).statistic), 4)))
    log(f"  {t}: n={len(cids)} coexp={rows[-1]['coexp_vs_atac']} emb_partial={rows[-1]['emb_partial']} "
        f"attn_partial={rows[-1]['attn_partial']} attn_vs_coexp={rows[-1]['attn_vs_coexp']}")

def mean(k): return round(float(np.mean([r[k] for r in rows])), 4) if rows else None
summary = dict(gap=GAP, per_type=rows, markers_excluded=True,
               mean=dict((k, mean(k)) for k in ["coexp_vs_atac", "emb_partial", "attn_partial", "attn_vs_coexp"]))
json.dump(summary, open(f"{OUT}/pertype_fm_v2.json", "w"), indent=2)
log(f"=== PER-TYPE FM means: {summary['mean']} ===")
log(f"SAVED {OUT}/pertype_fm_v2.json")
