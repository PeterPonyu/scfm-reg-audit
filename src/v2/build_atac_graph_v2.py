...
OUT = _ext_out if _ext_out else f"{ROOT}/results/v2"
...
if os.path.exists(cpath):
    ...
    with mp.Pool(NPROC, initializer=_winit, initargs=(MEME, mot_order, MOTIF_P, HG38)) as pool:
        for k, hits in pool.imap_unordered(_wscan, args, chunksize=64):
            ...
            now = time.time()
            if done == 1 or done % 10000 == 0 or (now - last_hb) >= 60:
                rate = done / max(now - t0, 1e-6)
                eta = (nRel - done) / max(rate, 1e-9)
                log(f"motif heartbeat {done}/{nRel} peaks ({100.0 * done / max(nRel, 1):.1f}%) "
