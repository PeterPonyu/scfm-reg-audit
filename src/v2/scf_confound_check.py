#!/usr/bin/env python
"""Confound-control gate for the scFoundation cross-modal result (same rigor as ko_confound_check.py /
confound_regression_v2.py — required before trusting any positive partial). CPU."""
...
G_scf = np.load(f"{OUT}/G_scf_pooled.npz")["G"]
...
a, co, scf = G_atac[ii, jj], G_co[ii, jj], G_scf[ii, jj]
...
json.dump(out, open(f"{OUT}/scf_confound_check.json", "w"), indent=2)
for k, v in out.items(): print(k, v)
print("SAVED scf_confound_check.json")
