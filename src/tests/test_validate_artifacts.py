...
against synthetic roots, plus one live check that every MANIFEST.json record
matches the bundled file it pins.
"""
import json
import os
import sys
...
class TestBundledManifest(unittest.TestCase):
    """Live check of the shipped manifest: every pinned record must still match."""
    ...
    def test_every_record_matches_its_file(self):
        import hashlib
        for record in self.manifest["files"]:
            path = Path(CAPSULE_ROOT) / record["path"]
            with self.subTest(path=record["path"]):
                self.assertTrue(path.exists(), f"{record['path']} listed but missing")
                self.assertEqual(record["bytes"], path.stat().st_size)
                self.assertEqual(hashlib.sha256(path.read_bytes()).hexdigest(),
                                 record["sha256"])

    def test_python_sources_are_listed(self):
        listed = {record["path"] for record in self.manifest["files"]}
        for path in Path(CAPSULE_ROOT).rglob("*.py"):
